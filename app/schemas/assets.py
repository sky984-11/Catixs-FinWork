...
class AssetCabinetBase(BaseModel):
    ...
    rental_start_u: int = Field(1, example=20)
    rental_end_u: int = Field(42, example=25)
    width_mm: int = Field(600, example=600)
    depth_mm: int = Field(1000, example=1000)
    power_allocation_kw: float = Field(0, example=4.0)
    power_overage_rate: str = Field("", example="RM180/0.1kW")
    pdu_spec: str = Field("", example="2x 24 ways SPN rPDU c/w 20 C13 & 4 C19 Power Socket")
...
class AssetDeviceBase(BaseModel):
    ...
    status: int = Field(0, description="设备状态：0-空闲，1-使用，2-待维护，3-故障，4-下架，6-预留", example=0)
    ...
    attributes: dict[str, Any] = Field(
        ...
        example={"CPU型号": "Intel Xeon Gold 5118", "CPU数量": "2", "CPU核心数": "24", "内存总数": "128G", "磁盘总数": "2T*2", "IPMI用户名": "admin"},
    )
    remark: str = Field("", example="")
