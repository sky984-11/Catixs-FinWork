...
供应商联系人页面按公司、销售、账单、NOC 角色汇总联系信息，支持筛选、新增、编辑、删除。公司角色维护 `company_email`、`company_phone`；NOC 角色维护 `noc_email`、`noc_phone` 和多行 `noc_contact`；销售、账单角色用多行维护姓名、邮箱、电话等。编辑仅提交 `id`、`name` 与所选角色字段；删除公司联系信息清空公司邮箱/电话，删除 NOC 信息同时清空 NOC 邮箱/电话，其他字段保持原值。不建立重复联系人数据。
